...
{
	//初始化过程参考手册
	const ADS_1256_CMD_t cmd = ADS1256_CMD_SELFCAL;				// 启动失调和增益自校准
	const ADS1256_DRATE_t drate = ADS1256_DRATE_10SPS; 			// 10k采样率
	const ADS1256_GAIN_t gain = ADS1256_GAIN_1;					// 正负5V最大值
	const ADS1256_CLOCK_t clock = CLOCK_ON;						// 时钟不分频输出
	const ADS1256_SENSOR_CURRENT_SOURCE_t sensor = SENSOR_OFF;	// 关闭传感器检测电流源
  	while(ADS1256_DRDY);
	CS_0();
